...
export default function BillListProduct({
  ...
  const handleAddToCart = async (product) => {
    try {
      ...
      // Gọi callback từ DetailHoaDon để thêm sản phẩm vào chi tiết hóa đơn
      if (onAddProduct) {
        onAddProduct(productToAdd);
      }

      // Nếu không có callback (chế độ bình thường), xử lý như cũ
      if (!onAddProduct) {
    ...
    <>
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="p-4 text-2xl font-bold bg-amber-600 text-white rounded-t-lg flex gap-2 opacity-90 items-center">
          ...
          Danh sách sản phẩm
          {isEditing && (
            <span className="text-sm bg-red-500 px-2 py-1 rounded ml-2">
              Chế độ chỉnh sửa
            </span>
          )}
        </div>
