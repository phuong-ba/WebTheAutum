...
const ShiftCheckMiddleware = ({ children }) => {
  ...
  const checkActiveShift = async () => {
    ...
    // Allow managers/admins to access all screens without starting a shift
    const userRole = (localStorage.getItem("user_role") || "").toString().trim().toLowerCase();
    if (userRole === "quản lý" || userRole === "admin" || userRole.includes("quản lý")) {
      setIsChecking(false);
      setHasActiveShift(true);
