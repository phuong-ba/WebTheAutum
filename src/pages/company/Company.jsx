...
} from "@/redux/slices/hangSlice";
import { updatePagination, updateAdvancedFilters, resetHangState } from "@/redux/slices/hangSlice";
import FilterCompany from "./FliterCompany";
import {
  ToggleLeftIcon,
  ToggleRightIcon,
...
export default function Company() {
  ...
  // Lấy state từ Redux - an toàn với optional chaining
  const hangState = useSelector((state) => state?.hang);

  // Fallback values
  const rawData = hangState?.data || null;
  const status = hangState?.status || "idle";
  const error = hangState?.error || null;
  const reduxPagination = hangState?.pagination || {
    current: 1,
    pageNo: 0,
    pageSize: 10,
    totalElements: 0,
  ...
  const reduxAdvancedFilters = hangState?.advancedFilters || {
    searchText: "",
    maNhaSanXuat: "",
    tenNhaSanXuat: "",
    ngayTao: null,
  ...
  const handleChangeStatus = (record) => {
    if (!record?.id) {
      return messageApi.error("Thông tin hãng không hợp lệ");
    }
    ...
    modal.confirm({
      ...
      content: `Bạn có chắc muốn ${action.toLowerCase()} hãng "${record.tenNhaSanXuat}"?`,
      okText: action,
      cancelText: "Hủy",
      async onOk() {
  ...
  const handleConfirmAdd = async () => {
    ...
    try {
      const maNhaSanXuat = addFormValues.maNhaSanXuat?.trim().toUpperCase();
      const tenNhaSanXuat = addFormValues.tenNhaSanXuat?.trim();
      ...
      messageApi.success(`Đã thêm hãng "${tenNhaSanXuat}" thành công!`);
      handleRefreshCurrentPage();
    } catch (error) {
      messageApi.error(error?.message || "Thêm hãng thất bại!");
    } finally {
      setIsAdding(false);
  ...
  const handleTableChange = (newPagination) => {
    ...
    dispatch(
      fetchFilterHang({
        ...
        maNhaSanXuat: reduxAdvancedFilters.maNhaSanXuat || "",
        tenNhaSanXuat: reduxAdvancedFilters.tenNhaSanXuat || "",
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <Button type="primary" danger onClick={() => dispatch(resetHangState())}>
            Thử lại
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="bg-white flex flex-col gap-3 px-4 py-5 rounded-lg shadow">
        <div className="font-bold text-4xl text-[#E67E22]">Quản lý hãng</div>
        <CompanyBreadcrumb />
      </div>
      ...
      <div className="bg-white rounded-lg shadow mb-6 overflow-hidden mt-6">
        <div className="bg-[#E67E22] text-white px-6 py-3">
          <div className="font-bold text-2xl text-white">Bộ lọc hãng</div>
        </div>
        <div className="p-4">
          <FilterCompany showAddModal={showAddModal} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="bg-[#E67E22] text-white px-6 py-3">
          <div className="font-bold text-2xl text-white">Danh sách hãng</div>
        </div>
        <Table
          columns={columns}
          ...
          pagination={{
            ...
            showTotal: (total, range) =>
              `${range[0]}-${range[1]} của ${total} hãng`,
          }}
          onChange={handleTableChange}
          locale={{ emptyText: "Không có dữ liệu hãng" }}
      ...
      >
        <Form form={addForm} layout="vertical" className="mt-6">
          ...
          >
            <Input placeholder="VD: Nike, Adidas, Gucci..." disabled={isAdding} />
          </Form.Item>
      ...
      >
        <div className="flex flex-col items-center gap-4 p-4">
          <h2 className="text-xl font-bold text-center">Xác nhận thêm hãng</h2>
          {addFormValues && (
            <div className="w-full bg-gray-50 rounded-lg p-4">
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <div className="font-medium text-gray-700 w-32">Tên hãng:</div>
                  <div className="font-semibold text-gray-900">{addFormValues.tenNhaSanXuat}</div>
                </div>
          ...
          <p className="text-gray-600 text-center mt-2">Bạn có chắc muốn thêm hãng này vào hệ thống?</p>
